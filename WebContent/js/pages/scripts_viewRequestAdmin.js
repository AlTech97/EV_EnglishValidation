...
$( document ).ready(function() {		
	$( document ).on( "submit", "#saveSurname", function() {
		...
		if(idUser != undefined && idUser.length > 0 && newSurname != undefined){
			$(".preloader").show();
	...
	$( document ).on( "click", ".changeSurname", function() {
		...
		if(idUser != undefined && idUser.length > 0 && surname != undefined && surname.length > 0){			
			$(".preloader").show();
			...
			text += '	<input type="text" class="form-control" id="surname" placeholder="Cognome" minlength="1" maxlength="20" value="'+surname+'" required>';
			text += '</div>';

			footer += '<button type="button" class="btn btn-default" data-dismiss="modal">Chiudi</button>';
			footer += '<button type="submit" class="btn btn-default btn-success confirm" data-iduser="'+idUser+'" >Salva</button>';
						
			$("#defaultModal form").attr("id", "saveSurname");			
	...
	$( document ).on( "submit", "#saveName", function() {
		var idUser = $("#saveName .confirm").data("iduser");
		var newName = $("#saveName #name").val();
		if(idUser != undefined && idUser.length > 0 && newName != undefined){
			$(".preloader").show();
	...
	$( document ).on( "click", ".changeName", function() {
		...
		if(idUser != undefined && idUser.length > 0 && name != undefined && name.length > 0){			
			$(".preloader").show();
			...
			text += '	<input type="text" class="form-control" id="name" placeholder="Nome" minlength="1" maxlength="20" value="'+name+'" required>';
			text += '</div>';

			footer += '<button type="button" class="btn btn-default" data-dismiss="modal">Chiudi</button>';
			footer += '<button type="submit" class="btn btn-default btn-success confirm" data-iduser="'+idUser+'" >Salva</button>';
						
			$("#defaultModal form").attr("id", "saveName");			
			$("#defaultModal .modal-title").html(title);
			$("#defaultModal .modal-body").html(text);
			$("#defaultModal .modal-footer").html(footer);			
